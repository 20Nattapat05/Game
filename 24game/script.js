...
function tryApplyOperation() {
  ...
  if (op === "sqrt") {
    ...
    if (selectedIndexes.length === 2) {
      // ใช้ลำดับคลิกจริง: ตัวแรกที่คลิก = a, ตัวที่สอง = b
      ...
      if (op === "+") {
        result = a + b;
        expression = `${formatNum(a)} + ${formatNum(b)} = ${formatNum(
          result
        )}`;
      } else if (op === "-") {
        result = a - b;
        ...
        if (Math.abs(a) > 100 || Math.abs(b) > 5) {
          setStatus(
            "ยกกำลังเลขนี้อาจใหญ่เกินไป ขอให้เลือกคู่เลขอื่น",
            false,
            true
          );
          return;
...
function checkWinOrContinue() {
  ...
  if (currentNumbers.length === 1) {
    ...
    if (Math.abs(val - 24) < 1e-6) {
      setStatus(
        `เยี่ยม! คุณทำได้เป็น 24 (ผลลัพธ์ = ${formatNum(val)}) 🎉`,
        true
      );
    } else {
      setStatus(
        `ยังไม่ใช่ 24 (ได้ ${formatNum(
          val
        )}) ลองสุ่มด่านใหม่หรือเริ่มด่านนี้ใหม่ดู`,
        false,
        true
      );
    }
    ...
    setStatus(
      "เลือกเลข 2 ตัว แล้วเลือกเครื่องหมาย (√ ใช้เลือกเลขตัวเดียว)"
    );
  }
}
